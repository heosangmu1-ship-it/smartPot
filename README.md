...
이 프로젝트의 **백엔드/프론트엔드 코드는 Anthropic Claude(Claude Code)와의 페어 프로그래밍으로 작성** 되었습니다.
정직성을 위해 미리 밝혀둡니다.

다만, **다음 부분은 전적으로 제가 직접 결정하고 구성했습니다:**

- **하드웨어 구성** — ESP32-S3 보드 + SHT30 온습도 센서 + 정전식 토양 수분 센서 v2.0 을 직접 배선/캘리브레이션
- **인프라/기술 도입 결정** — 이전부터 다뤄왔던 MQTT(Mosquitto), Redis, InfluxDB, Grafana, Tailscale 등을 이번 IoT 파이프라인 목적에 맞게 **재구성**
- **아키텍처 설계 의사결정** — Hot path / Cold path 분리, Redis Pub/Sub 기반 SSE fan-out, 1분 단위 집계로 차트 노이즈 제거 등 **모든 구조 결정은 본인의 판단**
- **문제 정의와 개선 방향** — "차트가 노이즈로 이상하다 → 1분 집계로 다운샘플링", "센서가 안 잡힌다 → 핀 변경 + raw 전압 디버깅" 같은 디버깅 흐름

즉 **"무엇을, 왜, 어떻게 만들 것인가"는 제가 직접 설계했고, 코드 작성 효율을 위해 AI를 활용한** 형태입니다.
면접에서는 모든 설계 결정과 코드 흐름을 직접 설명할 수 있도록 준비했습니다.
...
면접 대비용 의사결정 정리. 각 항목은 **"왜 이렇게 했나요?"** 에 답할 수 있도록 작성.
...
- **이 프로젝트로 어필하고 싶은 역량**:
  스마트팜 도메인의 IoT 데이터 파이프라인을 **설계 → 구현 → 디버깅 → 개선** 까지 직접 경험.
  Spring Boot 3 + React 18 + TypeScript + MQTT + 시계열 DB 라는 회사 스택과 정확히 맞물림.

- **AI 협업에 대한 생각**:
  AI 코딩 도구는 이미 실무의 일부입니다. 중요한 건 **AI에게 무엇을 시킬지 설계하는 능력** 과
  **나온 코드를 설명/디버깅/수정할 수 있는 이해도** 라고 봅니다.
  이 프로젝트를 만들면서 모든 아키텍처 결정과 기술 선택의 이유를 직접 정리해 두었고,
  코드 한 줄 한 줄을 면접에서 설명할 수 있도록 준비했습니다.
